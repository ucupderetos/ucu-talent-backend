...
| 6 | GET | `/vacancy-application/detailed?vacancyId={id}` | Listar postulaciones de una vacante con datos del alumno, la vacante y la empresa | 🔒 dueño (empresa dueña de la vacante) o rol `ADMIN` | — (query `vacancyId`: `@NotBlank`) | `List<ApplicationListItemResponse>` | `200` | `400` · `403` no es la empresa dueña ni ADMIN · `404` no existe la vacante |
| 7 | GET | `/vacancy-application/detailed` | Listar todas las postulaciones con datos del alumno, la vacante y la empresa | 🔒 rol `ADMIN` | — | `List<ApplicationListItemResponse>` | `200` | `403` no es ADMIN |
| 8 | GET | `/vacancy-application?studentProfileId={id}` | Listar por perfil de alumno | 🔒 rol `ADMIN` | — (query `studentProfileId`: `@NotBlank`) | `List<VacancyApplicationResponse>` | `200` | `400` · `403` no es ADMIN |
| 9 | GET | `/vacancy-application?status={status}` | Listar por estado | 🔒 rol `ADMIN` | — (query `status`: `VacancyApplicationStatus`) | `List<VacancyApplicationResponse>` | `200` | `400` enum inválido · `403` no es ADMIN |
| 10 | PUT | `/vacancy-application/{id}` | Actualizar el estado por id | 🔒 + dueño (empresa dueña de la vacante) | `UpdateVacancyApplicationRequest` | `VacancyApplicationResponse` | `200` | `400` · `403` no es la empresa dueña · `404` no existe · `409` transición inválida (retrocede) |
| 11 | PATCH | `/vacancy-application/{id}/accept` | Marcar la postulación como aceptada (`accepted: true`) | 🔒 + dueño (empresa dueña de la vacante) | — (path `id`) | `VacancyApplicationResponse` | `200` | `403` no es la empresa dueña · `404` no existe |
| 12 | DELETE | `/vacancy-application/{id}` | Eliminar postulación por id | 🔒 + dueño (alumno postulante) | — (path `id`) | — (vacío) | `204` | `403` no es el postulante · `404` no existe |
| 13 | GET | `/vacancy-application/status-summary` | Totales de postulaciones por estado | 🔒 rol `ADMIN` | — | `VacancyApplicationStatusSummaryResponse` | `200` | `403` no es ADMIN |
