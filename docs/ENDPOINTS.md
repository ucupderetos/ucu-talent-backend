...
| 7 | GET | `/vacancy-application/vacancy/{vacancyId}/detailed` | Listar postulaciones de una vacante con datos del alumno, la vacante y la empresa | 🔒 dueño (empresa dueña de la vacante) o rol `ADMIN` | — (path `vacancyId`: `@NotBlank`) | `List<ApplicationListItemResponse>` | `200` | `400` · `403` no es la empresa dueña ni ADMIN · `404` no existe la vacante |
